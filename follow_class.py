...
# follow 함수에서 self가 팔로우 하는 유저 리스트에 another_user를 넣고, 
# another_user를 팔로우 하는 유저 리스트에 self를 넣고, 
# 그렇게 생성된 리스트들의 길이를 리턴하면 된다고 생각.

# 한 가지 확신이 없었던 부분은 
# self.following_list.append(another_user)
# another_user.followers_list.append(self)
# 다음 부분으로 .이 두 개 나온다는 점, self와 another_user의 위치가 바뀌는 점이 헷갈렸었다.
# self.following_list.append(another_user)은 (self.following_list).append(another_user) 이렇게 생각하니 이해가 됐고,
# self 또한 self 라는 이름에 현혹되지 않고 변수라고 생각하니 위치가 바뀌는 것도 이해가 됐다.
